		...
		public function apply(bmd:BitmapData):BitmapData {
			...
			for(var y:int=0; y<bmdDes.height; y++) {
				for(var x:int=1; x<bmdDes.width-1; x++) {
					...
					var k2:int = limit(Number(bmdDes.getPixel(X+isOdd, y)&0xFF)+Number(remain*6));
					var k3:int = limit(Number(bmdDes.getPixel(X-isOdd, y+1)&0xFF)+Number(remain*3.5));
					var k4:int = limit(Number(bmdDes.getPixel(X, y+1)&0xFF)+Number(remain*5.0));
					var k5:int = limit(Number(bmdDes.getPixel(X+isOdd, y+1)&0xFF)+Number(remain)*1.5);
